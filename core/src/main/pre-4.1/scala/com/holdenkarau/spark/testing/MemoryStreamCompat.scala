 ...
 *
 * This is the pre-4.1 variant: MemoryStream lives in
 * org.apache.spark.sql.execution.streaming and is built from a SQLContext.
 */
private[testing] object MemoryStreamCompat {
  type Stream[T] = MemoryStream[T]

  def create[T: Encoder](spark: SparkSession): Stream[T] =
    MemoryStream[T](implicitly[Encoder[T]], spark.sqlContext)
}
