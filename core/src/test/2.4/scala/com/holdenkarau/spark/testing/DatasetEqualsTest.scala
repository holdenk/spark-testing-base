 ...
 * This tests the new assertDatasetEquals API which supports both Dataset[Row] (DataFrames)
 * and strongly-typed Datasets without needing to extend DatasetSuiteBase.
 */
class DatasetEqualsTest extends AnyFunSuite with DataFrameSuiteBase {
  ...
  test("assertDatasetEquals works with strongly-typed Dataset") {
    import sqlContext.implicits._
    val list = List(TestPerson("Holden", 2000, 60.0), TestPerson("Hanafy", 23, 80.0))
    val dataset1 = sc.parallelize(list).toDS
    val dataset2 = sc.parallelize(list).toDS

    // Should work with strongly-typed Datasets
    assertDatasetEquals(dataset1, dataset2)
  }

  test("assertDatasetEquals fails with unequal strongly-typed Dataset") {
    import sqlContext.implicits._
    val list1 = List(TestPerson("Holden", 2000, 60.0), TestPerson("Hanafy", 23, 80.0))
    val list2 = List(TestPerson("Holden", 2000, 60.0), TestPerson("Hanafy", 23, 80.1))
    val dataset1 = sc.parallelize(list1).toDS
    val dataset2 = sc.parallelize(list2).toDS
  ...
  test("assertDatasetApproximateEquals works with strongly-typed Dataset") {
    import sqlContext.implicits._
    val list1 = List(TestPerson("Holden", 2000, 60.0), TestPerson("Hanafy", 23, 80.0))
    val list2 = List(TestPerson("Holden", 2000, 60.1), TestPerson("Hanafy", 23, 80.1))
    val dataset1 = sc.parallelize(list1).toDS
    ...
    // Should work with tolerance on strongly-typed Datasets
    assertDatasetApproximateEquals(dataset1, dataset2, 0.2, Duration.ZERO, _.show())
  }
  ...
  test("assertDatasetApproximateEquals with strongly-typed timestamp Dataset") {
    import sqlContext.implicits._
    val list1 = List(TestMagicTime("Holden", Timestamp.valueOf("2018-01-12 20:41:32")))
    val list2 = List(TestMagicTime("Holden", Timestamp.valueOf("2018-01-12 20:41:49")))
    ...
    // Should work with timestamp tolerance on strongly-typed Datasets
    assertDatasetApproximateEquals(dataset1, dataset2, 0.0, Duration.ofSeconds(20), _.show())
  ...
  test("assertDatasetEquals with empty strongly-typed datasets") {
    import sqlContext.implicits._
    val emptyDS1 = sc.parallelize(List[TestPerson]()).toDS
    val emptyDS2 = sc.parallelize(List[TestPerson]()).toDS

    // Should work with empty strongly-typed datasets
    assertDatasetEquals(emptyDS1, emptyDS2)
  }
}
